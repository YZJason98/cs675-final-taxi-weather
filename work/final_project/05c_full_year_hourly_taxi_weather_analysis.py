...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Join NYC Yellow Taxi trips for a configurable date range with "
...
def main():
    ...
    spark = (
        ...
        .appName("cs675-full-year-hourly-taxi-weather-analysis")
    ...
    # ---------------------------------------------------------
    # Clean taxi data for the requested date range
    # ---------------------------------------------------------
    ...
    # ----------------------------------------------------------
    # Full-year monthly and seasonal analytical results
    # ----------------------------------------------------------

    monthly_weather_summary = (
        joined
        .filter(
            F.col("weather_condition").isNotNull()
        )
        .groupBy(
            "pickup_year",
            "pickup_month",
        ...
        .agg(
            ...
            F.round(
                F.avg("card_tip_percentage"),
                2,
            ).alias("avg_card_tip_percentage"),
            F.round(
                F.avg("temperature_c"),
                2,
